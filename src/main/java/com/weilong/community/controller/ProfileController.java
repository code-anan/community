...
public class ProfileController {
    ...
    public String profile(@PathVariable(name="action") String action, Model model, HttpServletRequest request,
        ...
        if("questions".equals(action)){
            ...
            List<NotificationDto> notificationList=notificationService.getNotificationList(user,page,size);
            ReplyPagination replyPagination=notificationService.getReplyPagination(user,page,size);
            model.addAttribute("notificationList",notificationList);
            model.addAttribute("section","reply");
            model.addAttribute("sectionName","回复我的问题");
            model.addAttribute("replyPagination",replyPagination);
        }
        return "profile";
    }
